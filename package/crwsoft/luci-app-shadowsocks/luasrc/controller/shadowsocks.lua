...
function act_status()
	...
	result.ss_tunnel = luci.sys.call("pidof %s >/dev/null" % "ss-tunnel") == 0
	luci.http.prepare_content("application/json")
	luci.http.write_json(result)
end

function get_pipecmd(cmd, default)
	local result = ""
...
function act_ping()
	...
	result.ping = get_pipecmd("ping -c 1 -W 1 %q 2>&1|grep -o 'time=[0-9]*.[0-9]'|awk -F '=' '{print$2}'" % luci.http.formvalue("domain"), "--")
	luci.http.prepare_content("application/json")
	luci.http.write_json(result)
end
